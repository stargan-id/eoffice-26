...
export function getMockPassphrase(): string {
  const passphrase = process.env.ESIGN_MOCK_PASSPHRASE;
  if (!passphrase) {
    // throw new Error('Missing ESIGN_MOCK_PASSPHRASE in environment variables.');
    return 'NOT-SET';
  }
  return passphrase;
}
